...
namespace OpenKh.Tools.ModsManager.Services
{
    public static class ModsService
    {
        ...
        public sealed record InstallResult(bool OverwroteExistingMod)
        {
            public static InstallResult Installed { get; } = new(false);
            public static InstallResult Overwritten { get; } = new(true);
        }
        ...
        public static Task<InstallResult> InstallMod(
            string name,
            bool isZipFile,
            bool isLuaFile,
            ...
            string branchName = null)
        {
            ...
            {
                return InstallModFromGithub(name, progressOutput, progressNumber, branchName);
            }
        ...
        public static InstallResult InstallModFromZip(
            string fileName,
            Action<string> progressOutput = null,
            Action<float> progressNumber = null)
        {
            ...
            var installResult = PrepareInstallPathAsync(modPath, modName).GetAwaiter().GetResult();
        ...
        public static async Task<InstallResult> InstallModFromGithub(
            ...
            string branchName = null)
        {
            ...
            var existingModPaths = modPaths.Where(Directory.Exists).ToList();
            var installResult = InstallResult.Installed;
            if (existingModPaths.Count > 0)
            {
                if (await ConfirmOverwriteAsync(repositoryName))
                {
                    installResult = InstallResult.Overwritten;
                    foreach (var existingModPath in existingModPaths)
        ...
        public static InstallResult InstallModFromLua(string fileName)
        {
        ...
        public static async Task<InstallResult> PrepareInstallPathAsync(string modPath, string modName)
        {
            if (!Directory.Exists(modPath))
                return InstallResult.Installed;
            if (!await ConfirmOverwriteAsync(modName))
                throw new ModAlreadyExistsExceptions(modName);

            CleanModFilesCore(modPath);
            return InstallResult.Overwritten;
        }

        private static async Task<bool> ConfirmOverwriteAsync(string modName)
        {
            if (_messageDialogService == null)
                throw new InvalidOperationException("ModsService.Initialize must be called before an overwrite confirmation is required.");

            var result = await _messageDialogService.ShowAsync(new MessageDialogRequest(
                $"A mod with the name '{modName}' already exists. Do you want to overwrite the mod install.",
                "Warning",
        ...
        // The legacy Helpers.Handle implementation is UI-coupled. Keep the
        // service's existing swallow-and-log behavior locally until error
        // presentation is migrated behind a separate neutral contract.
        private static void Handle(Action action)
        {
            ...
            {
                Log.Warn("Mods service operation failed: {0}", ex);
            }
        }

        private static T Handle<T>(Func<T> action)
        {
            try
            {
                return action();
